...
const DisplaySection: React.FC<DisplaySectionProps> = ({ selectedGender }) => {
  ...
  return (
    <div className="relative h-full pt-[2rem]">
          <div id="companion_1" className={`flex items-center justify-center absolute left-0 top-[10rem] h-full`}>
          ...
          <div id="center_stage" className="border-white w-full h-full">
              <div className="image-wrapper p-7">
                  <img id="client_img" 
                  className={`absolute w-[11rem] left-1/2 top-[12rem] -translate-x-1/2 transition-opacity duration-500 ${isFadingOut ? 'opacity-0' : (shouldBeVisible ? 'opacity-100' : 'opacity-0')}`} src={`/displaySection/${currentGender}.png`} alt="Display" />
              </div>
          </div>
          <div id="companion_2" className="flex items-center justify-center absolute right-0 top-[10rem] h-full">
              <div className="image-wrapper">
