...
export const COMPANION_SCREEN_MAPPER: Record<string, { modeText: string; statuses: Record<string, { textToDisplay: string; textInsideModal: string }>; instructions: Record<string, { textToDisplay: string; modalTexts: Record<string, string> }> }> = {
  ...
  [ACTIVITY_MODES.CAFE]: {
    modeText: 'Cafe',
    statuses: {
      [ACTIVITY_STATUS.QUEUE]: { textToDisplay: 'Once you get close, press [Call for Payment]', 
      ...
      [ACTIVITY_STATUS.PAYMENT_CALL]: { textToDisplay: 'Client will come and make Payment', 
      textInsideModal: 'Press YES to call Client to come and make PAYMENT. Change STATUS to [Waiting For Item] after that' },
      [ACTIVITY_STATUS.WAIT_ITEM]: { textToDisplay: 'Take order to Client once Ready', 
      ...
      [ACTIVITY_STATUS.WAIT_OP]: { textToDisplay: 'Wait for Instruction Message', 
